...
namespace dps
{
	IOM_DOH::IOM_DOH( const std::string& mdmname, const unsigned short iomidx ):IOM( mdmname, iomidx )
	{
	...
	void IOM_DOH::Connect( discsignals::DiscreteBundleManager* bman )
	{
		for (int c = 0; c < 3; c++)
		{
	...
	bool IOM_DOH::LoadState( const char* line )
	{
	...
	void IOM_DOH::SaveState( FILEHANDLE scn ) const
	{
		std::string str = "IOM" + std::to_string( iomidx );
		char cbuf[64];
	...
	bool IOM_DOH::busSCU( const unsigned char addr, const unsigned char task, unsigned short& data )
	{
	...
	void IOM_DOH::Output( const unsigned short addr )
	{
		unsigned short tmp = driver[addr];
		for (int i = 0; i <= 15; i++)
		{
			port[addr][15 - i].SetLine( (tmp & 0x0001) ? 28.0f : 0.0f );
			tmp >>= 1;
		}
		return;
	}

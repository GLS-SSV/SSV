...
namespace dps
{
	...
	void OMSTVCCMD_SOP::OnPostStep( double simt, double simdt, double mjd )
	{
		...
		WriteCOMPOOL_IS( SCP_FF1_IOM2_CH2_DATA, ReadCOMPOOL_IS( SCP_FF1_IOM2_CH2_DATA ) | ((Lsel == 0) ? 0x0006 : 0x0000) );// L OMS TVC: PRI ENABLE 1 & 2
		WriteCOMPOOL_IS( SCP_FF2_IOM2_CH2_DATA, ReadCOMPOOL_IS( SCP_FF2_IOM2_CH2_DATA ) | ((Lsel == 1) ? 0x0006 : 0x0000) );// L OMS TVC: SEC ENABLE 1 & 2
		WriteCOMPOOL_IS( SCP_FF4_IOM2_CH2_DATA, ReadCOMPOOL_IS( SCP_FF4_IOM2_CH2_DATA ) | ((Rsel == 0) ? 0x0006 : 0x0000) );// R OMS TVC: PRI ENABLE 1 & 2
		WriteCOMPOOL_IS( SCP_FF3_IOM2_CH2_DATA, ReadCOMPOOL_IS( SCP_FF3_IOM2_CH2_DATA ) | ((Rsel == 1) ? 0x0006 : 0x0000) );// R OMS TVC: SEC ENABLE 1 & 2
		return;
	}

...
The IUS can be installed in the payload bay in 2 possible positions: the forward position or the aft position. Figure \ref{fig:payload_volume_ius} shows the allowable payload envelope for both IUS positions. While the aft IUS position allows a payload with greater volume to be carried, it should only be used when necessary due to OV c.g. limits.\\
...
All these parameters are configured in the mission (see Section \ref{sec:mission-configuration}).
...
The Mission Editor can estimate the required SRM propellant load for a mission, based on the target orbital parameters and payload mass. More details are provided in Section \ref{sec:mission-configuration}.
...
%TODO\\
Inhibits are placed on the operation of the RCS and of the SRM-1 motor, as to protect the OV. At deployment, timers are started to remove those inhibits. The status of those timers, as well as the remaining RCS propellant, is displayed in the SSV\_IUS MFD (Ctrl+T).
...
After SRM-1 burnout, its separation is done by pressing the ''Ctrl+G'' key combination. After 1$^{st}$ stage separation, the Extendable Exit Cone in the SRM-2 will automatically deploy.\\
...
For IUS payload developers, the payload adapter must be 2.89 meters in diameter at the IUS-end, to correctly interface with the stage. The payload adapter can be a solid tapered cone as shown in Figure \ref{fig:ius} or a grid structure. The center of the payload adapter mesh should be at the center of the IUS-end. SSV includes a demonstration payload adapter for interfacing the IUS with the SSV\_DemoSat vessel.
\\
...
The payload adapter is specified in the Centaur vessel section of the mission file (Section \ref{sec:mission-configuration}). For Centaur payload developers, the payload adapter must be 2.74 meters in diameter at the Centaur-end, to correctly interface with the stage. The payload adapter can be a solid tapered cone as shown in Figures \ref{fig:centaurG} and \ref{fig:centaurGP} or a grid structure. The center of the payload adapter mesh should be at the center of the Centaur-end. SSV includes a demonstration payload adapter for interfacing the Centaur with the SSV\_DemoSat vessel.\\

...
When entering MM 104 and MM 105, the PEG-4 parameters for the OMS-1 and OMS-2 burns, respectively, are already filled with values defined in the mission configuration (see Section \ref{sec:mission-configuration}). OMS-1 automatic maneuver load is currently not implemented.\\
...
\subsubsection{TIME}
This system display, which is only available in orbit, allows the user to configure parameters related to time in the GPCs and MTUs. Currently the only functional ITEMs are 3-8 and 23.\\
The implementation of the Mission Time timers (ITEMs 3-8) is limited for now, and their values are not saved when exiting the simulation.
...
This display is used in GNC MM 201 and MM 202 to compute rendezvous burns. In real life, the state vectors for the rendezvous target are uploaded from Mission Control. In SSV, the name of the target vessel is defined in the mission file (see Section \ref{sec:mission-configuration}). It is then kept in the scenario file, allowing the target to be (manually) changed if there is more than one rendezvous target.\\
...
ITEM 41 selects the landing site, ITEMs 3 and 4 select either the primary or secondary runway.
The implemented version of HORIZ SIT supports a landing site table of 90 runways, grouped into 45 sites of 2 runways each.
ITEM 6 downmodes from an overhead approach to a straight-in approach, ITEM 7 toggles between the Nominal Entry Point (NEP) or the Minimum Entry Point (MEP), ITEM 8 switches the aim point between nominal and close, and ITEM 39 switches between nominal, short and ELS (Emergency Landing Site) speedbrake configurations for final approach.
These parameters all affect the entry guidance, so they are usually set before Entry Interface (EI).\\
The landing sites for a mission are defined in the mission file (see Sections \ref{sec:mission-configuration} and \ref{sec:landing-sites}).
\newpage
...
The Primary C\&W system alerts the crew via the C\&W light matrix on Panel F7 and the Master Alarm lights and sound. The Backup C\&W system alerts the crew with fault messages in the CRT displays, and relies on the Primary C\&W system to illuminate the "Backup C\&W Alarm" light on Panel F7, and to generate the Master Alarm sound or Systems Management tone.\\
The parameters of the Primary C\&W system can have their limits changed or have their checks inhibited via controls on Panel R13U.\\
...
\subsection{Orbiter Docking System (ODS)}
The Orbiter Docking System (ODS) allows the OV to dock to a space station. It consists mainly of the Russian-built Androgynous Peripheral Docking System (APDS), which is mounted on top of the External Airlock and handles the mechanical attachment with another vessel.\\
The original design of the APDS avionics, flown in the Mir missions, performed an auto docking sequence following capture, to perform ring alignment, retraction, hook closing and latch release. This sequence was eventually disabled due to problems in mechanically disengaging the brakes (dampers), and the crew had to manually perform each step via the APDS control panel. SSV implements the original version of the APDS avionics, but the auto docking sequence will be stopped if the manual procedure is followed, thus both options are currently available.\\
Due to the method used for docking ring (and docking port) motion, the docked stack rotation rates will be changed during ring motion.\\
There is no implementation of the docking ring dynamics (either caused by docking forces or mechanical issues), so the ring is always aligned. Also, there is no implementation of the pyro system for hook release, or the telemetry system and SPEC 167.\\
...
The CCTV cameras are configurable in the mission file. There are 2 types of cameras currently supported: the older models, identified as "-506/-508", and the more recent models "CTVC/ITVC". Currently they are functionally similar, with the difference being visual: the "-506/-508" has gold foil insulation, while the "CTVC/ITVC" has silver foil. In addition, the "CTVC/ITVC" can have a LED illuminator fitted around the lens, providing a light source in the field-of-view. The payload bay cameras are normally located at the corners of the PLB, but they can be mounted on fixed payloads, e.g., in mission STS-107, camera B was mounted on the aft bulkhead of the SpaceHab module.\\
...
For a "graceful degradation" in other graphics clients, Virtual Cockpit positions (see Section \ref{sec:nav-vc}) also exist for all the cameras, with the exception of the keel camera.
\newpage
...
The OBSS has a sensor package on the tip to record TPS condition and 2 Grapple Fixtures (GF) for the RMS to maneuver it. Also in the tip, there is an attachment for a Portable Work Platform (PWP).\\
During launch and landing it is mounted on the starboard MPM pedestals as a MPM Payload. The OBSS can easily be selected for a mission using the Mission Editor (see Section \ref{sec:mission-configuration}).\\
...
$\Rightarrow$ 1981, the original STS-1 pad, with original red FSS (Fixed Service Structure) and GVA vent pipes, but missing original SSWS (Sound Suppression Water System);\\
$\Rightarrow$ 1982, new GVA GOX vent pipes;\\
$\Rightarrow$ 1983, RBUS (Rolling Beam Umbilical System) porch added, and gray FSS;\\
...
The SSV Crawler is compatible with any launcher platform as long as the attachment point has the compatibility identifier "XMLP". The attachment point on the launcher platform should be on the very bottom of the platform.
\end{multicols*}
\newpage
...
\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_World.png}
  \caption{Runway locations around the world}
  \label{fig:RW_World}
\end{figure}
...
\noindent
SSV contains a comprehensive set of those landing sites, implemented as Orbiter Simulator surface bases, allowing them to be targeted for landing.\\
The available landing sites are listed in "<Orbiter installation>\textbackslash Config\textbackslash SSV\_RunwayDB.csv", and can be viewed in the Landing tab of Mission Editor (see Section \ref{sec:mission-configuration}), where the landing sites for the mission can be selected.
...
\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_AmericaNE.png}
  \caption{Runway locations in Northeast America}
  \label{fig:RW_AmericaNE}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_AmericaNW.png}
  \caption{Runway locations in Northwest America}
  \label{fig:RW_AmericaNW}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_Florida.png}
  \caption{Runway locations in Florida}
  \label{fig:RW_Florida}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_PacificAmericaS.png}
  \caption{Runway locations in South America and Eastern Pacific}
  \label{fig:RW_PacificAmericaS}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_Europe.png}
  \caption{Runway locations In Europe}
  \label{fig:RW_Europe}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_Africa.png}
  \caption{Runway locations In Africa}
  \label{fig:RW_Africa}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=0.99\hsize]{RW_AsiaSE.png}
  \caption{Runway locations in Asia and Oceania}
  \label{fig:RW_AsiaSE}
\end{figure}
\newpage
...
\begin{multicols*}{2}
\subsection{Xenon Lights}
\label{ssec:xenon-lights}
\noindent
The Xenon Lights provide a light source for some ground operations (during rollout to, or at, the pad), but mainly to illuminate the initial part of the runway during night landings.\\
The function of the lights can be enabled and disabled by pressing the Ctrl+K keys. When enabled, they will turn on automatically during the night, and turn off during the daytime.\\
They can also be pointed in a fixed direction, or they can track the OV (when it is closer than 1km, useful for rollouts). To maintain a fixed direction, the pan and tilt (in degrees) of the lights must be specified in the scenario with the parameter "PAN TILT <pan> <tilt>" in the SSV\_Xenon\_Light vessel.\\
The user can use the Mission Editor (see Section \ref{sec:mission-configuration}) to install Xenon Lights at selected runways.

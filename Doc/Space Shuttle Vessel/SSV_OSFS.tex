...
\subsection{Orbiter Docking System}
The target vessel's docking port position should be in-plane with the top surface of the structural ring of the Androgynous Peripheral Docking System (APDS).
\\
The original design of the APDS avionics, flown in the Mir missions, performed an auto docking sequence following capture, to perform ring alignment, retraction, hook closing and latch release. This sequence was eventually disabled due to problems in mechanically disengage the brakes (dampers), and the crew had to manually perform each step via the APDS control panel. SSV implements the original version of the APDS avionics, but the auto docking sequence will be stopped if the manual procedure is followed, thus both options are currently available.
\\
